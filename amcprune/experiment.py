    ...
    with open(path, "w", encoding="utf-8") as stream:
        ...
        for unit in selected:
            stream.write(
                ...
                f"linear_flops={unit.get('linear_flops')} "
                f"attention_flops={unit.get('attention_flops')} "
                f"kv_cache_cost={unit.get('kv_cache_cost')} "
                f"activation_cost={unit.get('activation_cost')} "
